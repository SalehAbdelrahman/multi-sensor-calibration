...
Node convert<lidar_detector::Configuration>::encode(const lidar_detector::Configuration & config) {
	...
	node["spatial_edge_filter"]      = Node(config.spatial_edge_filter);
...
bool convert<lidar_detector::Configuration>::decode(const Node & node, lidar_detector::Configuration & config) {
	...
	// Handle optional spatial_edge_filter for backward compatibility
	if (node["spatial_edge_filter"]) {
		config.spatial_edge_filter = node["spatial_edge_filter"].as<lidar_detector::SpatialEdgeFilter>();
	} else {
		// Set default values for solid-state edge detection
		config.spatial_edge_filter.search_radius = 0.1f;
		config.spatial_edge_filter.min_neighbors = 5;
		config.spatial_edge_filter.curvature_threshold = 0.5f;
		config.spatial_edge_filter.distance_threshold = 0.02f;
	}
...
Node convert<lidar_detector::LidarParameters>::encode(const lidar_detector::LidarParameters & config) {
	...
	node["is_solid_state"] = Node(config.is_solid_state);
	return node;
}
...
bool convert<lidar_detector::LidarParameters>::decode(const Node & node, lidar_detector::LidarParameters & config) {
	...
	// Handle optional is_solid_state for backward compatibility
	if (node["is_solid_state"]) {
		config.is_solid_state = node["is_solid_state"].as<bool>();
	} else {
		config.is_solid_state = false; // default to mechanical LiDAR
	}
...
Node convert<lidar_detector::SpatialEdgeFilter>::encode(const lidar_detector::SpatialEdgeFilter & config) {
	Node node;
	node["search_radius"]        = Node(config.search_radius);
	node["min_neighbors"]        = Node(config.min_neighbors);
	node["curvature_threshold"]  = Node(config.curvature_threshold);
	node["distance_threshold"]   = Node(config.distance_threshold);
	return node;
}
bool convert<lidar_detector::SpatialEdgeFilter>::decode(const Node & node, lidar_detector::SpatialEdgeFilter & config) {
	config.search_radius        = node["search_radius"].as<float>();
	config.min_neighbors        = node["min_neighbors"].as<int>();
	config.curvature_threshold  = node["curvature_threshold"].as<float>();
	config.distance_threshold   = node["distance_threshold"].as<float>();
	return true;
}
