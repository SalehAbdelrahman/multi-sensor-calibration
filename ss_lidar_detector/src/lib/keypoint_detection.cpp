...
/// Spatial-based edge detection for solid-state LiDARs
pcl::PointCloud<Lidar::PointWithDist> spatialEdgeDetection(
	pcl::PointCloud<Lidar::PointWithDist> const & cloud,
	SpatialEdgeFilter const & config
) {
	pcl::PointCloud<Lidar::PointWithDist> edge_cloud;
	
	// Convert to PCL PointXYZ for neighbor search
	pcl::PointCloud<pcl::PointXYZ>::Ptr xyz_cloud(new pcl::PointCloud<pcl::PointXYZ>);
	for (size_t i = 0; i < cloud.size(); ++i) {
		xyz_cloud->push_back(pcl::PointXYZ(cloud[i].x, cloud[i].y, cloud[i].z));
	}
	
	// Setup KdTree for neighbor search
	pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
	tree->setInputCloud(xyz_cloud);
	
	// For each point, analyze local neighborhood
	for (size_t i = 0; i < cloud.size(); ++i) {
		std::vector<int> indices;
		std::vector<float> distances;
		
		// Find neighbors within radius
		if (tree->radiusSearch(xyz_cloud->at(i), config.search_radius, indices, distances) >= config.min_neighbors) {
			// Compute local statistics
			float mean_distance = 0.0f;
			float max_distance = 0.0f;
			
			for (size_t j = 1; j < indices.size(); ++j) { // skip first (self)
				mean_distance += std::sqrt(distances[j]);
				if (std::sqrt(distances[j]) > max_distance) {
					max_distance = std::sqrt(distances[j]);
				}
			}
			mean_distance /= (indices.size() - 1);
			
			// Edge detection based on distance variation
			float distance_variation = max_distance - mean_distance;
			
			// Mark as edge if distance variation exceeds threshold
			if (distance_variation > config.distance_threshold) {
				Lidar::PointWithDist edge_point = cloud[i];
				edge_point.distance = distance_variation; // store the variation as distance
				edge_cloud.push_back(edge_point);
			}
		}
	}
	
	if (edge_cloud.size() == 0) {
		throw pcl::PCLException("Spatial edge detection did not find any edge points.");
	}
	
	return edge_cloud;
}

/// Create edges point cloud for solid-state LiDARs
pcl::PointCloud<Lidar::PointWithDist> createSpatialEdgeCloud(
	pcl::PointCloud<Lidar::PointWithDist> const & cloud, 
	SpatialEdgeFilter const & config
) {
	return spatialEdgeDetection(cloud, config);
}

/// Filter points based on spatial density for solid-state LiDARs
pcl::PointCloud<pcl::PointXYZ> filterSpatialOutliers(
	pcl::PointCloud<Lidar::PointWithDist> const & cloud,
	SpatialEdgeFilter const & config
) {
	pcl::PointCloud<pcl::PointXYZ> out;
	
	// Convert to PointXYZ and apply density-based filtering
	for (size_t i = 0; i < cloud.size(); ++i) {
		// Simple threshold-based filtering for now
		if (cloud[i].distance > config.distance_threshold) {
			pcl::PointXYZ point;
			point.x = cloud[i].x;
			point.y = cloud[i].y;
			point.z = cloud[i].z;
...
pcl::PointCloud<pcl::PointXYZ> keypointDetection(pcl::PointCloud<Lidar::PointWithDist> const & in, Configuration const & config) {
	...
	pcl::PointCloud<pcl::PointXYZ> circles_cloud;
	pcl::PointCloud<Lidar::PointWithDist> edges_cloud;

	// Choose edge detection method based on LiDAR type
	if (config.lidar_parameters.is_solid_state) {
		// Solid-state LiDAR processing: spatial-based edge detection
		edges_cloud = createSpatialEdgeCloud(cloud_calibration_board, config.spatial_edge_filter);
		
		// Convert edge points directly to PointXYZ for circle detection
		circles_cloud = filterSpatialOutliers(edges_cloud, config.spatial_edge_filter);
		
		if (config.visualize) { 
			// Note: visualize function may need to be updated for solid-state
			// visualize(cloud_calibration_board, pattern, edges_cloud); 
		}
	} else {
		// Mechanical LiDAR processing: ring-based edge detection
		...
		edges_cloud = createEdgeCloud(cloud_calibration_board, config.cloud_edge_filter, average_distance_ring);
		...
		circles_cloud = filterOutliers(edges_rings);
	}

	// Circle3d fit for all four circles (same for both types)
	pcl::PointCloud<pcl::PointXYZ> pattern = processCircles(circles_cloud, cloud_without_ground_floor, config.circle_detection);
	if (config.visualize && !config.lidar_parameters.is_solid_state) { 
		visualize(cloud_calibration_board, pattern, edges_cloud); 
	}
