...
template <>
struct convert<lidar_detector::SpatialEdgeFilter> {
	static Node encode(const lidar_detector::SpatialEdgeFilter & config);
	static bool decode(const Node & node, lidar_detector::SpatialEdgeFilter & config);
};
